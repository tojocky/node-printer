...
module.exports.printStream = printStream;
...
function printStream(parameters) {
    var stream,
        printer,
        docname,
        type,
    ...
    //TODO: check parameters type
    if(printer_helper.preparePrintStream &&
        printer_helper.writeStreamChunk &&
        printer_helper.finishPrintStream) {
        try {
            var res = printer_helper.preparePrintStream(printer, docname, type, options);
            if(!res) {
                error(Error('Something wrong in preparePrintStream'));
            }
            stream.on('data', function (chunk) {
                var writeStatus = printer_helper.writeStreamChunk(chunk, printer);
                ///DEBUG
                console.log('printer: writeStatus:', writeStatus);
            });
            stream.on('end', function () {
                var finishStatus = printer_helper.finishPrintStream(printer);
                ///DEBUG
                console.log('printer: finishStatus:', finishStatus);
                success(res);
            });
