...
/// Return if the printer name is in the getPrinters array
module.exports.isPrinterInstalled = isPrinterInstalled;
...
/** Verify if printer is in the getPrinters array
 * @param printerName printer name to extract the info
 * @return boolean
 */
function isPrinterInstalled(printerName)
{
    if(printer_helper.getPrinters().find(p => p.name ==  printerName)){
        return true
    }
    return false;
}
