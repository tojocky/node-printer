const printer_helper = require('bindings')('node_printer');
...
const properties = ['completedTime', 'creationTime', 'processingTime'];
...
function getPrinter(printerName) {
    ...
    if (printer.jobs) {
        doubleToDateArray(printer.jobs);
    }
...
function getJob(printerName, jobId) {
    let getJobReturn = printer_helper.getJob(printerName, jobId);
    doubleToDate(getJobReturn);
    return getJobReturn;
}

function doubleToDate(returnObject) {
    if (process.platform !== 'win32') {
        properties.forEach(function(element) {
            if (returnObject[element] != undefined) {
                returnObject[element] = new Date(returnObject[element]);
            }
        });
    }
}

function doubleToDateArray(arrayOfObj) {
    if (process.platform !== 'win32') {
        properties.forEach(function(element) {
...
function getPrinters() {
    ...
    if (printers && printers.length) {
        ...
        for (i in printers) {
            if (printers[i].jobs) {
                doubleToDateArray(printers[i].jobs);
            }
...
function printFile(parameters) {
    ...
    if (printer_helper.printFile) {
        ...
        try {
            ...
            var res = printer_helper.printFile(
                filename,
                docname,
                printer,
                options
